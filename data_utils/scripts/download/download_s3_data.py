...
from data_utils.scripts.download.helper import S3Functions
...
class S3DataDownloader:
    ...
    def get_file_in_range(
        ...
        for publisher in self.publishers:
            logger.info(
                f"Downloading {data_type} data for publisher: {publisher} for dates: {start_date} to {end_date}"
            )
...
def main():
    ...
    args = parse_args()
    # logger.info(f"AWS_ACCES_KEY_ID: {AWS_ACCESS_KEY_ID}")
